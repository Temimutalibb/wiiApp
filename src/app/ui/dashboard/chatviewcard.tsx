...
export default  function  ChatViewCard({userID} :{userID: string | undefined}){
  ...
  const handleClick = () =>{
    setChatData([...chatData , 
      {conversation_id: Number(chatId), user_id:Number(userID),
        content:messageData
      }])
      ...
      {chatData?.map(({conversation_id,user_id,content,}) => (
        <li
         className={`p-1 w-1/2 m-1 
          ${user_id === numberId? "bg-green-400 mr-auto" : "bg-green-200 ml-auto text-right"}`}
        key={conversation_id}>{content}
        </li>
      ))
       }
       </ul> 
