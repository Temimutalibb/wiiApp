...
import { ChatMessage, chatViewState, checkUser, CheckUserSchema, MessageSchema, SignupFormSchema, SignupFormState, User, userCheckState } from './placeholder-data';
...
export async function signupAction(  state: SignupFormState, formData: FormData){
  ...
  try{
     ...
     const insertData = await sql<User>`INSERT INTO users ( username, email, password)
...
export async function  authenticateChatUser(prevState: userCheckState, formData: FormData){
   ...
   if(validatedFields.success){
      ...
      console.log(email)
      try{
         const userCheck = await sql<checkUser>`SELECT * FROM users WHERE email= ${email}`;
         const user = userCheck.rows[0]
         if (!user){
            return{
        ...
        const conversationResult = await sql<{id : number}>`INSERT INTO conversation_user(
...
export async function fetchChatData(id : any | undefined): Promise<ChatMessage[]> {
   try{
...
export async function message( formData: FormData){
   ...
   if(validatedFields.success){
      ...
      try {await sql`INSERT INTO messages(conversation_id, user_id, content,
...
export async function getMessages(id : any | undefined): Promise<chatViewState[]>{
   try{
  ...
  export async function getUserId(email: string | undefined): Promise<string | undefined> {
    try {
  ...
  export async function  test(formData:FormData){
   ...
   console.log(email, initiatingUserId )
  } 